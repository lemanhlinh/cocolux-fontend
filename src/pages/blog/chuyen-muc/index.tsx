    ...
    return (
        <div className='ccs-blog row'>
            <Head>
                ...
                <meta name='description' content='CocoLux luôn cập nhật nhanh chóng đầy đủ những tin tức, xu hướng làm đẹp được giới trẻ yêu thích nhất. Cung cấp những mẹo nhỏ tiện lợi hơn trong chăm sóc da, trang điểm giúp các nàng tiết kiệm thời gian nhưng vẫn hữu ích.' />
                                                    ...
                                                    ? (
                                                        categories.map((c: any) => (
                                                            <Link
                                                                ...
                                                                as={`/chuyen-muc/${c.slug}`}
                                                            >
                                                                <a className={`category-list-item ${c.parent_id ? 'child' : 'parent'} ${categoryId === c.id ? 'active' : null}`}>
                                                                <span> {c.name}</span>
                                                                </a>
                                                            </Link>
                                                        ))
                        ...
                        isFirstLoad
                            ...
                            : (
                                <>
                                    ...
                                    <div className='middle__endbox ds'>
                                        {
                                            articles.length
                                                ? (
                                                    articles.map((el: any) => (
...
Article.getInitialProps = async ( query ) => {
    ...
    const listHotProduct = await ItemAPI.list({
        skip: 0,
        limit: 5,
        is_top_hot: true
    })
    ...
    return ({ 
        topArticles: response.data, 
        articles: res.data, 
        currentPage: page, 
        totalRecord: res.count, 
        recommendList: responseRe.data,
        categories: categories.data,
        categoryId: +firstCategory?.id,
        items: listHotProduct.data
    } || null);
}
