    ...
    async function fetchData() {
        ...
        if (categories.length) {
            ...
            categories.map((cate: any = {}) => {
                if (
                    cate.id === category.id ||
                    category.path?.includes(cate.id)
                ) {
                    if (category?.parent_id) {
                        cate.children.map((sub: any) => {
                            const checked = sub.id === category.id;
                            if (sub.children && sub.children?.length) {
                                sub.children.map((child: any) => {
                                    child.checked = child.id === category.id;
                                    return child;
                                });
                            }
                            values.push({
                        ...
                        values.push({
                            ...
                            name: cate.name,
                            slug: cate.slug,
                            checked: cate.id === category.id,
                            options: [...cate.children]
                        });
                    }
                }
            });
                                                                                ...
                                                                                ? (
                                                                                    option.options.map((item: any) => (
                                                                                        <span
                                                                                            id={item.id}
                                                                                            key={item.id}
                                                                                            className={`item-group-title pl-10 ${item.checked ? 'active' : null}`}
                                                                                            onClick={() => onChangePage(item.slug)}
                                                                                        >
                                                                                            {item.name}
                                                                                        </span>
                                                                                    ))
